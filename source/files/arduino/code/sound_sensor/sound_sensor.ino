...
void setup()
{
  ...
  pinMode(LED_BUILTIN,OUTPUT);            // Define LED13 as an output port, to indicate digital trigger reached
  pinMode(led2,OUTPUT);
}
...
void loop(){
  ...
  if(analogValue>585)      // When the Sound Sensor sends signla, via voltage present, light LED13 (L)
  {
  ...
  if(analogValue<584)      // When the Sound Sensor sends signla, via voltage present, light LED13 (L)
  {
  ...
  delay(1);                  // Slight pause so that we don't overwhelm the serial interface
}
